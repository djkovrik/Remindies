...
class TimeExtensionsKtTest {
    ...
    fun `test plusPeriod for days`() {
        ...
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Daily(3), currentTimeZone).run {
            year.should.be.equal(2021)
            monthNumber.should.be.equal(1)
    ...
    fun `test plusPeriod for weeks`() {
        for (i in 1..100) {
            ...
            someWhereInTheMiddle.plusPeriod(RemindiePeriod.Weekly(i), currentTimeZone).run {
                dayOfWeek.should.be.equal(secondDate.dayOfWeek)
            }
        }
    }
    ...
    fun `test plusPeriod for month`() {
        ...
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(1), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(2), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(3), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(4), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(5), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(6), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(7), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(8), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(9), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(10), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(11), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
        someWhereInTheMiddle.plusPeriod(RemindiePeriod.Monthly(12), currentTimeZone).run { dayOfMonth.should.be.equal(14) }
    }

    @Test
    fun `test plusPeriod for years`() {
        for (i in 1..100) {
            firstDate.plusPeriod(RemindiePeriod.Yearly(i), currentTimeZone).run { year.should.be.equal(firstDate.year + i) }
            secondDate.plusPeriod(RemindiePeriod.Yearly(i), currentTimeZone).run { year.should.be.equal(secondDate.year + i) }
        }
    }
